...
///
/// This macro derives two new items based on the given function.
/// 1. A `fuzz_target!` is generated that can be used with `cargo fuzz`.
/// 2. A `proptest` is generated that can be tested using `cargo test`.
///
...
///
/// Similar to `cargo fuzz`'s regular targets, user of this macro is expected to be in a separate
/// `[[bin]]` target that only contains a single `#[fuzztest]` target.
///
/// The recommended structure for your project `foo` is to put your tests under `foo/fuzz/src/bin`
/// such that any `.rs` file will be [automatically
/// discovered](https://doc.rust-lang.org/cargo/reference/cargo-targets.html#target-auto-discovery)
/// by cargo. `foo/fuzz/Cargo.toml` should also contain `[package.metadata] cargo-fuzz = true` as
/// generated by `cargo fuzz`. Note that `cargo fuzz init` / `cargo fuzz add` generates the binary
/// targets with `test = false`, which is not what we want, so be sure to remove that line if you
/// use that tool.
///
/// Each file should start with `#![cfg_attr(fuzzing, no_main)]` since libfuzzer includes its own
/// `main` entry point, whereas for testing, we do not want `no_main` so that `cargo test` will run
/// with Rust's built-in test harness. To facilitate this configuration, this macro generates a `fn
/// main() { unreachable!() }`, which allows the generated `proptest` and / or any other tests you
/// add in the fuzzing binary can run properly with `cargo test`.
///
/// ## Example
///
/// ```no_run
...
/// `main` entry point, whereas for testing, we do not want `no_main` so that `cargo test` will run
/// with Rust's built-in test harness. To facilitate this configuration, this macro generates a `fn
/// main() { unreachable!() }`, which allows any other tests you add in the fuzzing binary can run
/// properly with `cargo test`.
#[proc_macro_attribute]
...
/// Define a property test, similar to using `proptest::proptest!` directly.
///
/// All input parameters of the given function must implement `arbitrary::Arbitrary`.
///
